...
class DomainGroupLdapMapperTest {
  ...
  static void init() {
    ...
    mapper = new DomainGroupLdapMapper(properties, new DomainRepositoryMock(properties));
  }
  ...
  void map(SoftAssertions softly) {
    ...
    source.addAttributes(
        new LdapAttribute(RepositoryConstants.LDAP_WHEN_CREATED, "20170520150034.000Z"),
        new LdapAttribute(RepositoryConstants.LDAP_WHEN_CHANGED, "20180621160135.000Z"),
        new LdapAttribute(RepositoryConstants.LDAP_SAM_ACCOUNT_NAME, "somename"),
        new LdapAttribute(
            "member",
