...
import org.bremersee.dccon.repository.transcoder.DnsRecordValueTranscoder;
...
public class DnsNodeLdapMapper extends AbstractLdapMapper implements LdaptiveEntryMapper<DnsNode> {
  ...
  public String mapDn(final DnsNode dnsNode) {
    Dn dn = new Dn(new RDn(new NameValue(getProperties().getDnsNodeRdn(), dnsNode.getName())));
    dn.add(new Dn(getProperties().buildDnsNodeBaseDn(zoneName)));
    return dn.format();
    /*
    return
        createDn(
